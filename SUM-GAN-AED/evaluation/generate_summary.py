...
def generate_summary(all_shot_bound, all_scores, all_nframes, all_positions):
    ...
    for video_index in range(len(all_scores)):
        # Get shots' boundaries Change points [number_of_shots, 2] - the boundaries refer to the initial number of
        # frames (before the subsampling)
        ...
        # Compute the importance scores for the initial *frame sequence* (not the subsampled one) (gia omades frames
        # me vasi ta positions)
        ...
        for i in range(len(positions) - 1):
            ...
            if positions[i] == len(
                    frame_init_scores):  #an to positions[i] einai iso me to length twn subsampled frames (px 205),
