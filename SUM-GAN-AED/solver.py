...
from layers2.summarizer import Summarizer
from layers2.discriminator import Discriminator
from utils import TensorboardWriter
# from feature_extraction import ResNetFeature

# labels for training the GAN part of the model
original_label = torch.tensor(1.0)
summary_label = torch.tensor(0.0)
...
class Solver(object):
    ...
    def build(self):
        ...
        # Build Modules
        self.linear_compress = nn.Linear(self.config.input_size, self.config.hidden_size)

        self.summarizer = Summarizer(input_size=self.config.hidden_size, hidden_size=self.config.hidden_size, num_layers=self.config.num_layers)
        
        self.discriminator = Discriminator(input_size=self.config.hidden_size, hidden_size=self.config.hidden_size, num_layers=self.config.num_layers)
        
        self.model = nn.ModuleList([self.linear_compress, self.summarizer, self.discriminator])

        if self.config.mode == 'train':
            # Build Optimizers
            self.s_e_optimizer = optim.Adam(list(self.summarizer.attn.parameters()) + list(self.summarizer.vae.e_lstm.parameters())
                + list(self.linear_compress.parameters()),
                lr=self.config.lr)

            self.d_optimizer = optim.Adam(list(self.summarizer.vae.d_lstm.parameters()) + list(self.linear_compress.parameters()),
                lr=self.config.lr)
    ...
    def train(self):
        step = 0
        fieldnames = ['epoch', 's_e_loss', 'd_loss', 'c_loss', 'recon_loss', 'prior_loss', 'sparsity_loss', 'gan_loss',
    ...
    def evaluate(self, epoch_i):
        ...
        for video_tensor, video_name in tqdm(self.test_loader, desc='Evaluate', ncols=80, leave=False):
            ...
            video_feature = Variable(video_tensor, volatile=True)
