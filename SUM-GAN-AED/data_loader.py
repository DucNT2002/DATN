...
class VideoData(Dataset):
    def __init__(self, mode, split_index, filename, split_filename):
        self.mode = mode
        self.name = 'tvsum'
        ...
        with open(self.split_filename[0]) as f:
...
def get_loader(mode, split_index, filename, split_file):
    if mode.lower() == 'train':
        vd = VideoData(mode, split_index, filename, split_file)
        return DataLoader(vd, batch_size=1)
    else:
        return VideoData(mode, split_index, filename, split_file)
